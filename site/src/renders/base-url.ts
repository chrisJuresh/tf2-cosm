 ...
 */
export function joinRenderUrl(base: string, path: string): string {
  return `${base.replace(/\/+$/, "")}/${path.replace(/^\/+/, "")}`;
}

/** Where one manifest path is served from under the configured base. */
export function renderUrl(path: string): string {
  return joinRenderUrl(RENDER_BASE_URL, path);
}
